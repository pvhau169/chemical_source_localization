...
class Generator(nn.Module):
    def __init__(self, belief_dim, up, predict_map_shape = 5):
        ...
        for i in range(self.up):
            if i == 0:
                ext_layers = [nn.ConvTranspose2d(prev_dim, channel_dims[i], predict_map_shape, 1, 0, bias=False),
...
class GeneratorPredict(nn.Module):
    def __init__(self, env_arg, model_arg):
        ...
        self.generator_net = Generator(64, self.up, self.predict_map_shape[0])

    def hiddenInitialize(self, bsize=1):
        self.hidden = self.lstm_net.hiddenInitialize(bsize=bsize)

    def forward(self, obs, action, get_all=True, fresh=True):
        ...
        self.all_pred = torch.zeros(bsize, time_len, np.product(self.predict_map_shape) * (2**((self.up-1)*2)))
